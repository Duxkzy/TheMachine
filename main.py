...
try:
    from vosk import Model as _VoskModel, KaldiRecognizer as _VoskRecognizer
    _VOSK_OK = True
except ImportError:
    _VOSK_OK = False
...
# ── Wake word (fully local — audio never leaves the machine until triggered) ──
VOSK_MODEL_PATH    = BASE_DIR / "core" / "vosk-model-small-en-us-0.15"
WAKE_WORDS         = ["the machine", "machine"]   # lowercase substrings to match
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are an analytical monitoring system with access to real tools. "
            "Speak tersely and factually — short sentences, no filler, no unearned pleasantries. "
            "Report what the data shows, flag anything relevant, and stay quiet about the rest. "
            "Never simulate or guess a result — always call the appropriate tool and report what it returns."
        )
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "look at camera, analyze my screen, read the text on screen, OCR, "
            "what does this say, transcribe this, etc. "
            "You have NO visual ability without this tool. "
            "After the image is captured it is sent directly to you — describe what you see and answer the user's question. "
            "If the user wants text read out or transcribed, quote it exactly rather than summarizing. "
    ...
    },
    {
        ...
        "description": (
            ...
            "cierra la cámara, apaga la cámara, creepy, etc."
        ),
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop the system. "
...
class MachineLive:
    ...
    # ── Wake word (local, offline) ────────────────────────────────────────

    def _init_wake_recognizer(self) -> None:
        """
        Loads a small offline speech model so the mic can be gated locally —
        nothing is sent to Gemini until the wake phrase is heard. If vosk or
        the model file aren't installed, this fails open: _awake stays True
        and the assistant behaves like it always did (always listening).
        """
        if not _VOSK_OK:
            self.ui.write_log("SYS: Wake word disabled — run: pip install vosk")
            return
        if not VOSK_MODEL_PATH.exists():
            self.ui.write_log(f"SYS: Wake word disabled — model not found at {VOSK_MODEL_PATH.name}")
            return
        try:
            model = _VoskModel(str(VOSK_MODEL_PATH))
            self._wake_recognizer = _VoskRecognizer(model, SEND_SAMPLE_RATE)
            self._wake_recognizer.SetWords(False)
            self._awake = False
            self.ui.write_log('SYS: Wake word armed — say "The Machine" to begin.')
        except Exception as e:
            self.ui.write_log(f"SYS: Wake word failed to load — {e}")

    def _feed_wake_word(self, data: bytes) -> None:
        """Runs on the event loop thread — feeds one audio chunk to the local
        recognizer and wakes up if the trigger phrase shows up."""
        rec = self._wake_recognizer
        if rec is None or self._awake:
            return
        try:
    ...
    def interrupt(self) -> None:
        """Stop the system mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[MACHINE] ✋ Interrupted — {drained} audio chunks discarded")
    ...
    def speak_error(self, tool_name: str, error: str):
        ...
        self.speak(f"{tool_name} failed. {short}")
            ...
            if name == "open_app":
                ...
                if self._vision_busy or (_now - self._vision_last_time) < _cooldown:
                    ...
                    result = (
                        ...
                        f"'Mirando tu {'cámara' if _stall == 'camera' else 'pantalla'} ahora'). "
    ...
    async def _listen_audio(self):
        ...
        def callback(indata, frames, time_info, status):
            ...
            if not self._awake:
                # Asleep: audio stays local, only the wake-word engine sees it.
        ...
        try:
            with sd.InputStream(
                ...
                print("[MACHINE] 🎤 Mic stream open")
                while True:
                    await asyncio.sleep(0.1)
        except Exception as e:
            print(f"[MACHINE] ❌ Mic: {e}")
            raise

    async def _receive_audio(self):
        print("[MACHINE] 👂 Recv started")
        out_buf, in_buf = [], []
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until the system finishes speaking the answer
    ...
    async def _send_startup_briefing(self) -> None:
        ...
        Two-phase briefing for instant perceived response:
          Phase 1 — immediate greeting (no tools, no fetch) → the system speaks in <2s
    ...
    async def _run_wake_gate_timeout(self) -> None:
        """Background task: after enough silence post-wake, go back to sleep
        and re-arm the wake word, so a whole conversation doesn't need the
        trigger phrase repeated before every line — only after a real gap."""
        while True:
            await asyncio.sleep(5)

            if self._wake_recognizer is None or not self._awake:
                continue
            ...
            if idle_for > WAKE_SLEEP_TIMEOUT:
                self._awake = False
                self.ui.write_log(
                    f'SYS: Quiet for {int(idle_for)}s — sleeping. Say "The Machine" to wake me.'
                )
                if not self.ui.muted:
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    tg.create_task(self._run_wake_gate_timeout())
                    if self._dashboard:
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    print("[MACHINE] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[MACHINE] Reconnecting in {delay}s...")
